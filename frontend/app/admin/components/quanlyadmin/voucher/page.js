...
export default function VoucherPage() {
        ...
        try {
          ...
          while (currentPage <= totalPages) {
            const response = await fetch(
              `https://wristlybackend-e89d41f05169.herokuapp.com/voucher/getAllVouchers?page=${currentPage}`
            );
  ...
  const handleToggleComment = async (id, trang_thai) => {
    try {
      const response = await fetch(
        `https://wristlybackend-e89d41f05169.herokuapp.com/voucher/getVoucherByStatus/${id}`,
        {
          method: "PUT",
        }
      );
  ...
  const fetchVouchers = async () => {
    ...
    try {
      const response = await fetch(
        `https://wristlybackend-e89d41f05169.herokuapp.com/voucher/getAllVouchers?page=${currentPage}&ma_voucher=${searchQuery}&limit=${itemsPerPage}`
      );
      if (!response.ok) {
  ...
  const deleteVoucher = async (id) => {
    ...
    try {
      const response = await fetch(
        `https://wristlybackend-e89d41f05169.herokuapp.com/voucher/deleteVoucher/${id}`,
        {
          method: "DELETE",
        }
      );
