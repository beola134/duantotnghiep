...
export default function TaiKhoan() {
  ...
  try {
    ...
    if (result.isConfirmed) {
      const response = await fetch(`https://wristlybackend-e89d41f05169.herokuapp.com/users/update/${id}`, {
        method: "PUT",
...
const exportToPDF = () => {
  ...
  const deleteUser = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`https://wristlybackend-e89d41f05169.herokuapp.com/users/delete/${id}`, {
          method: "DELETE",
        });
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch("https://wristlybackend-e89d41f05169.herokuapp.com/users");
        if (!response.ok) {
                  ...
                  <tbody>
                    {displayUsers.map((item) => (
                      <tr key={item._id}>
                        ...
                        <td style={{ textAlign: "center" }}>
                          <img src={`https://wristlybackend-e89d41f05169.herokuapp.com/images/${item.hinh_anh}`} alt={item.danh_muc} />
                        </td>
