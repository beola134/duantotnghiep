...
export default function DanhMuc() {
        ...
        try {
          ...
          await Promise.all(
            categories.map(async (item, index) => {
              ...
              const response = await fetch(`https://wristlybackend-e89d41f05169.herokuapp.com/images/${item.hinh_anh2}`);
              if (!response.ok) {
  ...
  const exportToPDF = async () => {
    ...
    for (const item of categories) {
      if (item.hinh_anh2) {
        const imageUrl = `https://wristlybackend-e89d41f05169.herokuapp.com/images/${item.hinh_anh2}`;
        try {
  ...
  const deleteCategory = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`https://wristlybackend-e89d41f05169.herokuapp.com/thuonghieu/delete/${id}`, {
          method: "DELETE",
        });
        if (!response.ok) {
                  ...
                  <tbody>
                    {displayCategories.map((item) => (
                      <tr key={item._id}>
                        ...
                        <td style={{ textAlign: "center" }}>
                          <img src={`https://wristlybackend-e89d41f05169.herokuapp.com/images/${item.hinh_anh2}`} alt={item.thuong_hieu} />
                        </td>
                        <td style={{ textAlign: "center" }}>
                          <Link
