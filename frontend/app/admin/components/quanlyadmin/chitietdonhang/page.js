  ...
  const fetchUsers = async (page = 1, limit = itemsPerPage, query = "") => {
    try {
      const response = await fetch(
        `https://wristlybackend-e89d41f05169.herokuapp.com/donhang/getAllOrderDetails?page=${page}&limit=${limit}&ten_san_pham=${query}`
      );
      ...
      if (result.isConfirmed) {
        try {
          ...
          const firstResponse = await fetch(
            `https://wristlybackend-e89d41f05169.herokuapp.com/donhang/getAllOrderDetails?page=${currentPage}&limit=1000&ten_san_pham=${encodeURIComponent(
              query
            )}`
          );
