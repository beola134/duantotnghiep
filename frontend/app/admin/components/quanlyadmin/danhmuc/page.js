...
export default function DanhmucPage() {
  ...
  const fetchDanhmuc = async () => {
    try {
      const response = await fetch(
        `https://wristlybackend-e89d41f05169.herokuapp.com/cate/allcate?page=${currentPage}&ten_danh_muc=${searchQuery}`
      );
      if (!response.ok) {
  ...
  const deleteDanhmuc = async (id) => {
    ...
    try {
      const response = await fetch(
        `https://wristlybackend-e89d41f05169.herokuapp.com/cate/deletecate/${id}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (result.isConfirmed) {
        try {
          ...
          while (currentPage <= totalPages) {
            const response = await fetch(
              `https://wristlybackend-e89d41f05169.herokuapp.com/cate/allcate?page=${currentPage}`
            );
            const data = await response.json();
                  ...
                  return (
                    <tr key={_id}>
                      ...
                      <td style={{ width: "10%", textAlign: "center" }}>
                        <img src={`https://wristlybackend-e89d41f05169.herokuapp.com/images/${hinh_anh}`} />
                      </td>
