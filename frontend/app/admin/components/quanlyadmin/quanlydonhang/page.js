 ...
 const exportToPDF = async () => {
  ...
  useEffect(() => {
    const fetchDonhang = async () => {
      try {
        const response = await fetch("https://wristlybackend-e89d41f05169.herokuapp.com/donhang/showAll");
        if (!response.ok) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          const response = await fetch(`https://wristlybackend-e89d41f05169.herokuapp.com/donhang/update/${id}`, {
            method: "PUT",
            headers: {
